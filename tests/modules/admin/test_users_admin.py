...
async def test_get_user_permissions_is_zone_scoped_for_view_only(db, agency):
    """I3 (final review): mirrors the Task 5 fix for `get_user` (ERR-ACL-002) — a
    view-only holder must not be able to read another user's PERSONAL GRANTS
    outside their own zone by id either, not just their card."""
    suffix = uuid.uuid4().hex[:6]
    org_a = Organization(
        kind="leshoz", code=f"pva-{suffix}", name={"uz_cyrl": "А"}, parent_id=agency.id
    )
    org_b = Organization(
        kind="leshoz", code=f"pvb-{suffix}", name={"uz_cyrl": "Б"}, parent_id=agency.id
    )
    db.add_all([org_a, org_b])
    await db.flush()
    ...
    async with make_client(app, lifespan=True) as client:
        auth_client(client, token, csrf)
        ok = await client.get(f"{API}/admin/users/{in_zone.id}/permissions")
        blocked = await client.get(f"{API}/admin/users/{out_of_zone.id}/permissions")
    assert ok.status_code == 200, ok.text
    assert blocked.status_code == 403, blocked.text
    assert blocked.json()["error"]["code"] == "ERR-ACL-002"


async def test_get_user_permissions_manage_holder_sees_out_of_zone(db, agency):
    suffix = uuid.uuid4().hex[:6]
    org_a = Organization(
        kind="leshoz", code=f"pma-{suffix}", name={"uz_cyrl": "А"}, parent_id=agency.id
    )
    org_b = Organization(
        kind="leshoz", code=f"pmb-{suffix}", name={"uz_cyrl": "Б"}, parent_id=agency.id
    )
    db.add_all([org_a, org_b])
    await db.flush()
    ...
    async with make_client(app, lifespan=True) as client:
        auth_client(client, token, csrf)
        r = await client.get(f"{API}/admin/users/{out_of_zone.id}/permissions")
    assert r.status_code == 200, r.text
...
async def test_create_user_with_sys_admin_role_by_non_sys_admin_rejected(db):
    """R5a (final review): `auth.users.manage` alone must not let a non-sys_admin
    manage-holder mint a sys_admin account — a privilege-escalation path that
    `_staff_role_or_422`'s existing checks (archived/applicant) did not cover."""
    _, token, csrf = await signed_in_with(db, USERS_MANAGE)
    await db.commit()
    app = create_app()
    async with make_client(app, lifespan=True) as client:
        ...
        r = await client.post(
            f"{API}/admin/users",
            json={
                "login": f"wannabe-{uuid.uuid4().hex[:8]}",
                "full_name": "X",
                "role_code": "sys_admin",
            },
        )
    assert r.status_code == 422, r.text
    assert r.json()["error"]["details"]["reason"] == "superuser_role_restricted"


async def test_create_user_with_sys_admin_role_by_sys_admin_allowed(db):
    suffix = uuid.uuid4().hex[:8]
    ...
    async with make_client(app, lifespan=True) as client:
        ...
        r = await client.post(
            f"{API}/admin/users",
            json={
                "login": f"newadmin-{suffix}",
                "full_name": "New Admin",
                "role_code": "sys_admin",
            },
        )
    assert r.status_code == 201, r.text
    assert r.json()["user"]["role_code"] == "sys_admin"
...
async def test_patch_user_to_sys_admin_role_by_non_sys_admin_rejected(db):
    """R5a (final review): the PATCH role-reassignment path shares
    `_staff_role_or_422` with create — same privilege-escalation guard applies."""
    _, token, csrf = await signed_in_with(db, USERS_MANAGE)
    ...
    async with make_client(app, lifespan=True) as client:
        auth_client(client, token, csrf)
        r = await client.patch(f"{API}/admin/users/{target.id}", json={"role_code": "sys_admin"})
    assert r.status_code == 422, r.text
    assert r.json()["error"]["details"]["reason"] == "superuser_role_restricted"
...
async def test_block_deleted_user_rejected(db):
    """I4 (final review): deletion is terminal (С23) — `block` must not accept a
    `deleted` account (it would otherwise leave it sitting in `blocked`, one
    `unblock` away from being resurrected)."""
    _, token, csrf = await signed_in_with(db, USERS_MANAGE)
    target = await make_user(db, status="deleted")
    await db.commit()
    app = create_app()
    async with make_client(app, lifespan=True) as client:
        auth_client(client, token, csrf)
        r = await client.post(f"{API}/admin/users/{target.id}/block", json={"reason": "x"})
    assert r.status_code == 422, r.text
    assert r.json()["error"]["details"]["reason"] == "wrong_status"


async def test_unblock_deleted_user_rejected(db):
    """I4 (final review): the direct resurrection path — `unblock` on a `deleted`
    account must not silently revive it."""
    _, token, csrf = await signed_in_with(db, USERS_MANAGE)
    target = await make_user(db, status="deleted")
    await db.commit()
    app = create_app()
...
async def test_unblock_active_user_rejected(db):
    _, token, csrf = await signed_in_with(db, USERS_MANAGE)
    target = await make_user(db)  # default status: active
    await db.commit()
    app = create_app()
    async with make_client(app, lifespan=True) as client:
        auth_client(client, token, csrf)
