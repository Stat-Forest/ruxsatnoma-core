...
async def export_notifications_xlsx(
    ...
    items, total, cap = await export.inbox_rows(db, actor=user, lang=lang, unread=unread)
    filename = f"bildirishnomalar-{business_today().isoformat()}.xlsx"
    return xlsx.xlsx_response(
        export.inbox_render(items, lang=lang), filename=filename, total=total, cap=cap
    )
