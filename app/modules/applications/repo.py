...
async def set_benefit_verification(
    ...
    *,
    ...
    """Write the verifier's decision and read the row back — `add_document`'s
    own shape (flush then refresh): `updated_at` is `onupdate=func.now()` and
    would otherwise stay EXPIRED for whatever the caller's response
    serializes (lesson: the row in memory is not what Postgres stored)."""
    application.benefit_verification_status = status
    application.benefit_verified_by = verified_by
    application.benefit_verified_at = verified_at
